...
def read_results(path, label=None, anomaly_ratio=0.2, dataset_name=None):
    event_hit_data = []
    dirs = [dir for dir in os.listdir(path) if label in dir] if label is not None else os.listdir(path)
    ...
    for dir in dirs:
        ...
        file_path = os.path.join(path, dir, "events", f"event_results_{dataset}_{anomaly_ratio}.txt")
...
def read_f1(path, label=None, anomaly_ratio=0.2, dataset_name=None):
    ...
    for dir in dirs:
        if dataset_name is not None:
            ...
            dataset = dir.split('_')[-1]
        file_path = os.path.join(path, dir, "f1", f"f1_{dataset}_{anomaly_ratio}.txt")
        if not os.path.exists(file_path):
            print(f"File {file_path} does not exist")
            continue
...
def read_anomaly_scores(path, label=None, anomaly_ratio=0.2):
    ...
    for dir in dirs:
        dataset = dir.split('_')[-1]
        file_path = os.path.join(path, dir, "results", f"results_{dataset}_{anomaly_ratio}.txt")
